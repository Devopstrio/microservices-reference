...
const DashboardLayout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const navItems = [
    { name: 'Service Topology', path: '/', icon: LayoutDashboard },
    { name: 'Identity Service', path: '/users', icon: Users },
    { name: 'Order Service', path: '/orders', icon: ShoppingCart },
    { name: 'Payment Service', path: '/payments', icon: CreditCard },
    { name: 'Inventory Service', path: '/inventory', icon: Package },
    { name: 'Notification Service', path: '/notifications', icon: Mail },
    { name: 'Saga Orchestration', path: '/sagas', icon: Workflow },
    { name: 'Gateway Stats', path: '/gateway', icon: Zap },
    { name: 'Observability', path: '/observability', icon: Activity },
    { name: 'Security & RBAC', path: '/security', icon: ShieldCheck },
    { name: 'System Metrics', path: '/metrics', icon: BarChart3 },
    ...
    <div className="flex h-screen bg-slate-950 text-slate-200 overflow-hidden font-sans">
      ...
      <aside className={`bg-slate-900 border-r border-slate-800 transition-all duration-300 ${sidebarOpen ? 'w-64' : 'w-20'} flex flex-col`}>
        <div className="p-6 flex items-center gap-4 border-b border-slate-800">
          ...
          {sidebarOpen && <span className="font-bold text-xl tracking-tight bg-gradient-to-r from-white to-indigo-400 bg-clip-text text-transparent">Micro.Ref</span>}
        </div>
          ...
          <div className="flex items-center gap-3 p-2 bg-slate-800/50 rounded-xl">
            ...
            {sidebarOpen && (
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold truncate">Platform Architect</p>
                <p className="text-xs text-slate-500 truncate">Cloud Native Hub</p>
              </div>
            )}
          </div>
        </div>
        ...
        <header className="h-16 bg-slate-900/50 backdrop-blur-md border-b border-slate-800 flex items-center justify-between px-6 z-10">
          <div className="flex items-center gap-4 flex-1">
            ...
            <div className="relative max-w-md w-full">
              ...
              <input 
                type="text" 
                placeholder="Search services, trace IDs, saga workflows, API routes..." 
